...
function createVideoElement(id, isLocal,istext) {
    var videoDiv = document.createElement("div");
    videoDiv.innerHTML = '<video id="' + id + '" autoplay ' + (isLocal ? 'muted' : '') + ' playsinline class="'+(istext=='huanzhe'?'yuancheng':'')+'"></video>';
...
function onRemoteStreamUpdate(info) {
    ...
    if (info.stream && info.stream.active === true) {
        ...
        var video = document.getElementById(id);
        if (!video) {
            video = createVideoElement(id,'','huanzhe');
        }
        video.srcObject = info.stream;
    } else {
...
function initRTC(opts) {
    window.RTC = new WebRTCAPI({
        ...
        "debug":{
            ...
            "vconsole": false, //是否展示 vconsole （方便在移动端查看日志）
