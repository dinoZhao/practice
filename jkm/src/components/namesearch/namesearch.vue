  ...
  <div id="ns">
    ...
    <div class="ns-ul-container" @click="recover" id="scroll_body" ref="scroll_body">
	  ...
	  <ul class="ns-letter-ul">
		  <li class="ns-letter-li" v-for="(item,index) in letterArr" :key="index"  :data-word="item" @click.stop="initialSearch">{{item}}</li>
	  </ul>
	  <!--新版锚点滑动通讯录--head-->
	  <div >
	  <ul v-for="(item,index) in regularAllPatients">
	  	<li v-for="(value,key) in item" v-if="value.length > 0">
	  		<div class="first-name-sign"  :ref="'anchor-'+key">{{key}}</div>
	  		 <ul class="ns-list-ul">
        <li class="ns-list-li" :class="{active:indexItem+key===personCurrentIndex}" v-for="(valueItem,indexItem) in value" :key="indexItem" :data-personId="valueItem.personId" @click.stop="personCurrentIndex = indexItem+key,searchSinglePatient($event)">
          <img :src="valueItem.headUrl||'http://ww1.sinaimg.cn/large/005SvFg2ly1g0tb4guu90j302i02ijr6.jpg'" alt="picture unloaded" class="ns-list-pic">
          <div class="ns-list-right">
            <div class="ns-list-right-top">{{valueItem.name}}</div>
            <div class="ns-list-right-bottom">
              <div class="ns-information">{{valueItem.sex==='0'?'女':'男'}}</div>
...
//		console.log(event.target.value);
  		...
  		if(event.target.value){
  		  vm.cachePatients.map(function(item){
  			if(item.name.indexOf(event.target.value)>-1){
//				console.log(event.target.value+'---'+item.name)
  				AllPatients.push(item);
  			}
  		})
  		vm.AllPatients = AllPatients;	
  		}
  		else{
  			vm.AllPatients = vm.cachePatients;	
  		}
...
//		console.log(event.currentTarget.dataset.personid);
  	...
  	initialSearch(event){
  		...
  		let vm = this;
//		  console.log(vm.$refs);
		   document.querySelector("#scroll_body").scrollTop = vm.$refs['anchor-'+event.currentTarget.dataset.word]?vm.$refs['anchor-'+event.currentTarget.dataset.word][0].offsetTop:'';
