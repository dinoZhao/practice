  			...
  			<div class="blood" v-if="istype=='blood'&&!isreuslt">
  				<div  class="process">
					...
					<div  class="line">
						...
						<div class="right" @click="startecg">
							开始测量
						</div>
					</div>
  			</div>
...
export default {
  ...
  methods:{
	...
	printFrom(){
		var vm = this;
		vm.show_print = true;
    	printingReport({recordId: vm.recordId,padDeviceCode: 'P1',personId: vm.personId,type: 'ECG'}).then(res=>{
			console.log(res)
		})
	},
	closePrint(){
...
<style>
	...
	.ecg,.blood,.oxygen,.glucose,.animalheat,.ecg-result,.blood-result,.glucose-result,.animalheat-result{
		/*display: none;*/
	}
	.startdetection .ecg{
 ...
 .ecg-result{
	...
	.ecg-result .line>div:first-child, .ecgfur .line>div:nth-child(2) {
	    -ms-flex-positive: 2;
	    flex-grow: 2;
	}
