<template>
  <div class="doctordetail"> 
  	...
  	<div class="doctordetail-head">
  		<img v-bind:src="result.nickUrl" class="head-icon" /><div class="head-conent">
  			<P><span class="name">{{result.name}}</span><span>{{result.profession}}</span></P>
  			<P>医院：<span>{{result.organization}}</span></P>
  			<P>擅长：<span>{{result.description}}</span></P>
  		</div>
  	</div>
  				...
  				<li class="list-item">
	  				<div class="item-left">
	  					...
	  					<template v-if="item.remain<2">
	  						<span class="few1" @click="gotoaffirm(item.schedulingTableId)" v-bind:data-id="item.schedulingTableId">剩&nbsp;<b>{{item.remain}}</b></span>
	  					</template>
	  					<template v-else-if="item.remain>2">
	  						<span class="more" @click="gotoaffirm(item.schedulingTableId)" v-bind:data-id="item.schedulingTableId">剩&nbsp;<b>{{item.remain}}</b></span>
	  					</template>
	  					<template v-else>
	  						<span class="zero" v-bind:data-id="item.schedulingTableId">约满</span>
	  					</template><img src="../../../assets/arrows-left.png" />
	  				</div>
	  			</li>
  			</template>
  		</ul>
...
export default {
  ...
  methods:{
  	gotoaffirm:function(id){
  		var vm = this;
  		window.location.href = 'affirmexpert.html?scid='+id+'&did='+vm.doctorId;
  	}
  },
  components:{
    headline,
